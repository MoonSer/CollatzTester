...
#include <cassandra.h>
#include <iostream>
constexpr std::string_view KeySpaceCreate = "CREATE KEYSPACE Collatz with replication = {'class':'SimpleStrategy', 'replication_factor' : 2};";
constexpr std::string_view KeySpaceUse = "USE Collatz;";
constexpr std::string_view CreateTable = "CREATE TABLE IF NOT EXISTS Collatz( first_member decimal, second_member decimal, sqs_sequence list<bigint>, is_cycle boolean, PRIMARY KEY((first_member, second_member, is_cycle)));";
constexpr std::string_view InsertQuery = "INSERT INTO Collatz (first_member, second_member, sqs_sequence, is_cycle) VALUES (?, ?, ?, ?);";
